        ...
        try:
            ...
            for blob in blob_list:
                if len(blob.name.split('/')[-1]) > 1:
                    ...
                    # concatenating all pages in one document
                    text_to_upload += annotation.text + '\n'
            # Uploading all pages in one document
            text_file = bucket.blob('raw-text-files/' + file_id + '.txt')
            text_file.upload_from_string(text_to_upload)
